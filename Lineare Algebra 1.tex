...
\begin{center}
\myItem[Satz 9.12 Basisergänzungssatz ]
\end{center}
V endlichdimensionaler K-VR, $(u_1,...,u_n)$ lin. unabh. Familie in V \\
...
\newpage
\begin{center}
\myItem[9.13 Zornsches Lemma]
\end{center}
Jede induktiv geordnete nichtleere Menge $(M, \leq)$ besitzt ein maximales Element. Hierbei heißt eine halbgeordnete Menge $(M, \leq)$ \textbf{induktiv geordnet} \\
$\Leftrightarrow$ Jede Teilmenge $T \subseteq M$, für die $(T, \leq)$ totalgeordnet ist, besitzt eine obere Schranke in $(M, \leq)$, d.h. es ex. ein $S\in M$ mit $t \leq S$ für alle $t \in T$ \\

Anm: Das Zornsche Lemma ist äquivalent zum Auswahlaxiom. \\

\begin{center}
\myItem[Satz 9.14]
\end{center}
$(u_j)_{j \in J}$ lin unabh. Familie in V \\
Dann kann $(u_j)_{j \in J} $ zu einer Basis von V ergänzt werden, d.h. es ex. eine Menge $I$, mit $I \subseteq J$ und eine Familie $(v_i)_{i \in I} $ mit $v_j = u_j$ für alle $j \in J$, sodass $(v_i)_{i \in I} $ eine Basis von V ist. \\
...
\section*{Matrizen}
\addcontentsline{toc}{section}{10§ Matrizen}

\myItem[Disclaimer:]
Viele triviale Definition mit vielen Indizes etc. sind hier ausgelassen worden und wurden auf Papier niedergeschrieben. \\
...
\addcontentsline{toc}{section}{§11 Summen von Untervektorräumen}
...
\addcontentsline{toc}{section}{Lineare Abbildungen}
